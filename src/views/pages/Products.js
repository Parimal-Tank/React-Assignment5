...
const Products = () => {
  ...
  const loadData = async (page , limit) => {
    const result = await axios(`https://dummyjson.com/products?skip=${(page-1)*limit}&limit=${limit}`);
  ...
  return (
    <div className="m-5">
      ...
      <div className="d-flex justify-content-center">
          <Pagination  justifyContent="center" count={numberOfPages} showFirstButton={true} showLastButton={true} onChange={(e, page) => handleChange(e,page)} />
      </div>
    </div>
  );
};
